...
export const extractLinks = (html: string, base: URL, visited: Set<string>) => {
	const { document } = parseHTML(html)
	const out: string[] = []
...
const crawl = (base: URL, max: number, options: DiscoverOptions = {}, robots?: RobotsRules) =>
	Effect.gen(function* () {
				...
				batch.map((item) =>
					tryFetch(item.url, options).pipe(
						Effect.map((r) => {
							...
							return { depth: item.depth, links: extractLinks(r.text, base, visited) }
						}),
					),
				),
				{ concurrency: 20 },
...
export const discover = (baseUrl: string, max: number, options: DiscoverOptions = {}) =>
	Effect.gen(function* () {
		...
		return yield* crawl(actual, max, crawlOptions, robots)
	})
